...
class TestWorkflows:
    ...
    def test_full_borrow_and_return_workflow(self, auth_client):
        """Test the complete lifecycle: create book -> create user -> borrow -> return."""
        # 1. Create a book
        book = auth_client.post(
            "/api/v1/books/",
            json={
                "title": "Test Book",
                "author": "Author",
        ...
        book_updated = auth_client.get(f"/api/v1/books/{book['id']}").json()
        ...
        # 6. Return the book
        returned = auth_client.put(f"/api/v1/loans/{loan['id']}/return").json()
        assert returned["status"] == "returned"

        # 7. Verify book is available again
        book_final = auth_client.get(f"/api/v1/books/{book['id']}").json()
        assert book_final["available_copies"] == 1

    def test_multiple_users_borrow_same_book(self, auth_client):
    ...
    def test_dashboard_stats_reflect_operations(self, auth_client):
        """Test that dashboard stats update correctly after operations."""
        # Initial stats
        stats = auth_client.get("/api/v1/stats/dashboard").json()
    ...
    def test_cannot_delete_book_with_active_loan(self, auth_client):
        ...
        response = auth_client.delete(f"/api/v1/books/{book['id']}")
        assert response.status_code == 409
    ...
    def test_deactivate_user_with_active_loan_fails(self, auth_client):
        ...
        response = auth_client.put(f"/api/v1/users/{user['id']}/deactivate")
        assert response.status_code == 409
